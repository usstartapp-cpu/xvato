<?php
/**
 * BridgeKit — REST API Endpoints
 *
 * Registers and handles all custom REST API routes:
 *   POST /bridgekit/v1/import   — Accept import from Chrome extension
 *   GET  /bridgekit/v1/status   — Connection test + site info
 *   GET  /bridgekit/v1/status/{id} — Check import job status
 *   GET  /bridgekit/v1/library  — List imported templates
 *
 * @package BridgeKit
 */
...
class RestAPI {
    ...
    public static function register_routes(): void {
        ...
        register_rest_route( BRIDGEKIT_REST_NAMESPACE, '/import', [
        ...
        register_rest_route( BRIDGEKIT_REST_NAMESPACE, '/status', [
        ...
        register_rest_route( BRIDGEKIT_REST_NAMESPACE, '/status/(?P<id>\d+)', [
            'methods'             => \WP_REST_Server::READABLE,
        ...
        register_rest_route( BRIDGEKIT_REST_NAMESPACE, '/library', [
            'methods'             => \WP_REST_Server::READABLE,
    ...
    public static function handle_import( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
        ...
        if ( empty( $payload['title'] ) ) {
            return new \WP_Error(
                'bridgekit_missing_title',
                __( 'Template title is required.', 'bridgekit' ),
                [ 'status' => 400 ]
            );
        }
        ...
        // If we have a download URL, schedule the import
        if ( ! empty( $payload['download_url'] ) ) {
            // Schedule async import
            wp_schedule_single_event(
                time(),
                'bridgekit_process_import',
                [ $post_id, $payload['download_url'] ]
            );

            // Also try to process immediately if possible
            // (some hosts run cron on page load)
            spawn_cron();

            return new \WP_REST_Response( [
                'success' => true,
                'message' => __( 'Import queued for processing.', 'bridgekit' ),
                'data'    => [
                    'job_id'  => $post_id,
                    'status'  => 'queued',
                    'title'   => $payload['title'],
        ...
        return new \WP_REST_Response( [
            'success' => true,
            'message' => __( 'Template entry created. Upload the ZIP file manually in the BridgeKit dashboard.', 'bridgekit' ),
            'data'    => [
                'job_id' => $post_id,
    ...
    public static function handle_status( \WP_REST_Request $request ): \WP_REST_Response {
        $data = [
            ...
            'library_count'     => wp_count_posts( BRIDGEKIT_CPT )->publish ?? 0,
        ];
    ...
    public static function handle_job_status( \WP_REST_Request $request ): \WP_REST_Response|\WP_Error {
        ...
        if ( ! $post || BRIDGEKIT_CPT !== $post->post_type ) {
            return new \WP_Error(
                'bridgekit_not_found',
                __( 'Import job not found.', 'bridgekit' ),
                [ 'status' => 404 ]
            );
        }
...
add_action( 'bridgekit_process_import', function ( int $post_id, string $download_url ) {
